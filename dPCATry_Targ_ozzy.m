...
trialNum_all=[];
trialNum_NB_all=[];
for record=1:length(Date)%:5%:5%5%1:6
...
%                     f_mean=nanmean(f,1);
%                     f0=(f0-nanmin(f_mean(:)))/(nanmax(f_mean(:))-nanmin(f_mean(:)));
                    for choice=1:2
                        u1=0;
                        for Opt1=[2,12,11]
                            u1=u1+1;
...
%                             if PreferD(ch,unit,1)==1
                                ID= I_choice & InfoTrial.Repeat==0 & InfoTrial.ContrastBG>2 &  InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==choice ; %& InfoTrial.ChoiceOpt==Opt1 ;
                                ID_NC= I_NC  & InfoTrial.Repeat==0 & InfoTrial.ContrastBG>2 &  InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==choice ; %& InfoTrial.ChoiceOpt==Opt1 ;
                                %end
%                             else
%                                 ID= I_choice & InfoTrial.Repeat==0 & InfoTrial.ContrastBG>2 &  InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==3-choice ; %& InfoTrial.ChoiceOpt==Opt1 ;
%                                 ID_NC= I_NC & InfoTrial.Repeat==0  & InfoTrial.ContrastBG>2 &  InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==3-choice ; %& InfoTrial.ChoiceOpt==Opt1 ;
%                                 
%                             end
...
for n=1:size(firingRates_all,1)%36,110%1:size(firingRates_all,1)
    for direction=1:2
        ...
        clear a00** a a_NC
        a=squeeze(nanmean(firingRates_all(n,direction,:,t>-500 & t<0,:),4))';
        a_NC=squeeze(nanmean(firingRates_NB_all(n,direction,:,t>-500 & t<0,:),4))';
...
figure(10)
subplot(221)
h0=hist(b_all(1,:),-0.3:0.03:0.3);
bar(-1:0.1:1,h0,'facecolor',[0.5 0.5 0.5]);hold on;
[h,p]=ttest(b_all(1,:));
h1=hist(b_all(1,b_all_sig(1,:)<0.05),-0.3:0.03:0.3);
bar(-1:0.1:1,h1,'facecolor',[0.2 0.2 0.2]);hold on;
axis([-0.3 0.3 0 35]);
title(['Choice RF',num2str(p,3)]);

subplot(222)
h0=hist(b_all(2,:),-0.3:0.03:0.3);
bar(-1:0.1:1,h0,'facecolor',[0.5 0.5 0.5]);hold on;
[h,p]=ttest(b_all(2,:));
h1=hist(b_all(2,b_all_sig(2,:)<0.05),-0.3:0.03:0.3);
bar(-1:0.1:1,h1,'facecolor',[0.2 0.2 0.2]);hold on;
axis([-0.3 0.3 0 35]);
...
subplot(223)
h0=hist(b_NC_all(1,:),-0.3:0.03:0.3);
...
subplot(224)
h0=hist(b_NC_all(2,:),-0.3:0.03:0.3);
