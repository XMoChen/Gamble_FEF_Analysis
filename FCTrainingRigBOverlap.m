...
function FCTrainingRigBOverlap
...
%     Eyelink( 'Message', 'Central Y %ld',Attention_Loc);
 ...
 while (i+1)<=round(length(Type_ID))
    ...
    i=i+1;
     ...
     Type_ID(i);
    t_overlap=t_overlap0+rand(1)*t_overlaprand; 
  ...
  if fix1==1
   ...
   %Check recording status, stop display if error
    ...
    RtSumR=[RtSumR;(ChoiceIn-FixOn)];
     ...
     if Direct<5 &  ((x_eye >= (center(1)+x_fp2-(window_choice/2)))&&(x_eye <= (center(1)+x_fp2+(window_choice/2)))&&(y_eye >= (center(2)-y_fp2-(window_choice/2)))&&(y_eye <= (center(2)-y_fp2+(window_choice/2))))
     choice=2;
     Eyelink('Message', 'ChoiceIn');
