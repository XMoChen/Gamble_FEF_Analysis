...
function FixationTrainingRigBForceMIB
...
try
    ...
    pixperdeg=ppd;%Number of pixels that equal one degree of visual angle
...
%      v_y_fp=[0, round(-6*ppd),round(-6*ppd)]-round(11.5*ppd);
     v_y_fp=[round(-6*ppd), round(-6*ppd),round(6*ppd), round(-6*ppd),round(6*ppd)];
...
% Connection with Eyelink if not in testing mode
	...
	BackGray=(white+black)/2;
	if round(BackGray)==white
		BackGray=black;
    end
      BackGray=159;
...
%%%%% To put MIB in buffer, not useful    
        textureIdx = MIBFrame(1);
        Screen('FillRect',w, BackGray);
        Screen('FillOval',w,fp_color, [center(1)-fpr+x_fp, center(2)-fpr-y_fp, center(1)+fpr+x_fp, center(2)+fpr-y_fp],pi/4); 
        t1Rect=[(x_res/2-size_rect_gabor/2+x_fp(1)) (y_res/2-size_rect_gabor/2-y_fp(1)) (x_res/2+size_rect_gabor/2+x_fp(1)) (y_res/2+size_rect_gabor/2-y_fp(1))];
        Screen('DrawTexture', w, textureIdx,[],t1Rect,45+(MIB_Direct-1)*90);   
%%%%%%
    Screen('FillRect',w, BackGray);
