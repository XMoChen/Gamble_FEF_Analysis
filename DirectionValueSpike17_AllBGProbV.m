...
%             Spike_BG_base00=Spike_BG_base00(101:end);
          ...
          if 1==1%hSig(unit_all,1)==0 & hSig(unit_all,3)==0 & hSig(unit_all,4)==0 & hSig(unit_all,5)==0% ...
            ...
            for ep=1:4
                ...
                elseif ep==3
                    t=-500:1001;
                    clim=[-500 1000 Spike_min Spike_max];
                else
                    t=-500:501;
                ...
                for d=1:2%4
                    u=0;
                    I= ContrastBG>0 & InfoTrial.Repeat==0 & InfoTrial.SaccadeStart>500 & GS_I==1 & InfoTrial.ChoiceD==d;
                ...
                for d=1:2%4
                    u=0;
                    I=  ContrastBG>0 & InfoTrial.Repeat==0 & InfoTrial.SaccadeStart>500 & GC_I==1 & InfoTrial.ChoiceD==d;
                    plotstd(t,Spike(I,:),char(Direction_Color(d)));hold on;
                    axis(clim);
                end
                xlabel(['From ',char(Epoch(ep)),' onset']);% title('Single');
                ...
                for d=1:2%4
                    ...
                    for v=[4,21,22,23,24,25]
                        u=u+1;
                        clear I
                        I=  ContrastBG>0 & InfoTrial.Repeat==0 & InfoTrial.SaccadeStart>500 & GS_I==1 & InfoTrial.ChoiceD==d & InfoTrial.ChoiceOpt==v;
                ...
                xlabel(['From ',char(Epoch(ep)),' onset']); %title('Choice');
                    ...
                    for v=[4,21,22,23,24,25]
                        u=u+1;
                        clear I
                        I= ContrastBG>0 & InfoTrial.Repeat==0 & InfoTrial.SaccadeStart>500 & GC_I==1 & InfoTrial.ChoiceD==d & InfoTrial.ChoiceOpt==v;
                        plotstd(t,Spike(I,:),char(V_Color(u)));hold on;
                    end
                    axis(clim);
                    box off;
...
Epoch={'BG','TG','SC','RT'};
%%%% Align on target onset
clim_min=0.4;%min([min(Spike_BG_All(:)),min(Spike_TG_All(:)),min(Spike_SC_All(:)),min(Spike_RT_All(:))]);
clim_max=0.4;
for ep=1:4
    clear Spike
