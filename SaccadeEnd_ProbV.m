function SaccadeEnd_ProbV(Spikefile,Eventfile,DirFig,Ch_num,name0,Bg_Lable)
%%%% dock the figures
...
PreferD=1;
if Bg_Lable==1
    GS_I=ContrastBG>1 & InfoTrial.TrialType==1 & InfoTrial.Targ2Opt==0;
    GC_I=ContrastBG>1 & InfoTrial.TrialType==1 & InfoTrial.Targ2Opt~=0;
else
    GS_I=ContrastBG==1 & InfoTrial.TrialType==1 & InfoTrial.Targ2Opt==0;
    GC_I=ContrastBG==1 & InfoTrial.TrialType==1 & InfoTrial.Targ2Opt~=0;
end
    ...
    for unit=2:length(l_ts(ch,:)) %3
        if l_ts(ch,unit)>5000
            ...
            if 1==1%hSig(unit_all,1)==0 & hSig(unit_all,3)==0 & hSig(unit_all,4)==0 & hSig(unit_all,5)==0% ...
                ...
                figure(unit_all)
                Epoch={'BG','TG','SC','RT'};
                Spike_max=max(nanmean(Spike_SC,2));
                Spike_min=min(nanmean(Spike_SC,2));
                ...
                clear Spike tt
                eval(['Spike=(Spike_SC-Spike_min)/(Spike_max-Spike_min);']);
                %                 Spike_max=1;
                %                 Spike_min=0.5;
                ...
                t=-500:1001;
                clim=[-300 800 Spike_min*0.8 Spike_max*2];
                ...
                for d=1:2%4
                    ...
                    for v=[4,21,22,23,24,25]
                        ...
                        if (v==4 | v==21 | v==25)
                            ...
                            SpikeSaccade(unit_all,d,u,:)=nanmean(Spike(I,:),1);
                ...
                for d=1:2%4
                    ...
                    for v=[4,21,22,23,24,25]
                        ...
                        if (v==4 | v==21 | v==25)
                            ...
                            SpikeSaccade(unit_all,d+2,u,:)=nanmean(Spike(I,:),1);
                            plotstd(t,Spike(I,:),char(V_Color(u)));hold on;
                            box off;set(gca,'TickDir','out')
                            axis(clim);
                ...
                print( FigHandle, '-djpeg', [DirFig,'Ch',num2str(ch),'Unit',num2str(unit),'_SaccadeEnd']);
                %
                %
...
%
h2=figure;
aclim=nanmean(SpikeSaccade,1);
clim=[-400 800 min(aclim(:)), max(aclim(:))];
for d=1:2
    subplot(2,4,d)
    
    plotstd(t,squeeze(SpikeSaccade(:,d,2,:)),'g');
    plotstd(t,squeeze(SpikeSaccade(:,d,1,:)),'m');
    plotstd(t,squeeze(SpikeSaccade(:,d,6,:)),'r');
    axis(clim);
    box off;set(gca,'TickDir','out')
    ...
    subplot(2,4,d+2)
    plotstd(t,squeeze(SpikeSaccade(:,d,3,:)),'k');
    plotstd(t,squeeze(SpikeSaccade(:,d,4,:)),'c');
    plotstd(t,squeeze(SpikeSaccade(:,d,5,:)),'b');
    axis(clim);
    box off;set(gca,'TickDir','out')
    title(['FC D',num2str(d)])
    ...
    subplot(2,4,d+4)
    plotstd(t,squeeze(SpikeSaccade(:,d+2,2,:)),'g');
    plotstd(t,squeeze(SpikeSaccade(:,d+2,1,:)),'m');
    plotstd(t,squeeze(SpikeSaccade(:,d+2,6,:)),'r');
    axis(clim);
    box off;set(gca,'TickDir','out')
    title(['Choice D',num2str(d)])
    
    subplot(2,4,d+6)
    plotstd(t,squeeze(SpikeSaccade(:,d+2,3,:)),'k');
    plotstd(t,squeeze(SpikeSaccade(:,d+2,4,:)),'c');
    plotstd(t,squeeze(SpikeSaccade(:,d+2,5,:)),'b');
    axis(clim);
    box off;set(gca,'TickDir','out')
    title(['Choice D',num2str(d)])
...
if Bg_Lable==1
    ...
    BG_SpikeSaccade=SpikeSaccade;
    save(Spikefile,'BG_SpikeSaccade','-append');
    print( h2, '-djpeg', [DirFig,'SaccadeEnd_',name0,'_BG']);
    print( h2, '-djpeg', [sumfig,'SaccadeEnd_',name0,'_BG']);
else
    suptitle(['NBG ',char(name0)]);
    NBG_SpikeSaccade=SpikeSaccade;
    save(Spikefile,'NBG_SpikeSaccade','-append');
    print( h2, '-djpeg', [DirFig,'SaccadeEnd_',name0,'_NBG']);
    print( h2, '-djpeg', [sumfig,'SaccadeEnd_',name0,'_NBG']);
end
