...
%     plot(t,nanmean(Spike(ContrastBG'==c & InfoTrial.FixationOff>500,:),1),char(Contrast_Color(c)));hold on;
% end
%         end
%     end
% end
...
if l_ts(ch,unit)>5000
 ...
 eval(['Targ_dmax=UnitInfo.Ch',num2str(ch),'Unit',num2str(unit),'.TargPreferD;']);   
  ...
  I1= InfoTrial.Repeat>=0 & InfoTrial.TargetOn>500 & GC_I==1 &  ContrastBG>1 & InfoTrial.DOpt(:,Targ_dmax)==18;
  I2= InfoTrial.Repeat>=0 & InfoTrial.TargetOn>500 & GC_I==1 &  ContrastBG>1 & InfoTrial.DOpt(:,Targ_dmax)==2;
...
subplot(2,2,1) 
 ...
 I= InfoTrial.Repeat>=0 & InfoTrial.FixationOff>500 & GS_I==1  ;
...
subplot(2,2,3) 
 ...
 I= InfoTrial.Repeat~=0 & InfoTrial.FixationOff>500 & GS_I==1  ;
 AllTarg.FC_ProV_RP(unit_all,u,:)=nan(1,1002);
 if sum(I)>5
 plotstd(t,Spike(I,:),char(Value_Color(u)));hold on;
 title({['Ch',num2str(ch),'Unit',num2str(unit)],'FC  Chosen'});  
 xlabel('From Target onset');
 axis(clim);  
 AllTarg.FC_ProV_RP(unit_all,u,:)=(nanmean(Spike(I,:),1)-Spike_min)/(Spike_max-Spike_min);
end
 
  subplot(2,2,2) 
  clear I
  I= InfoTrial.Repeat>=0 & InfoTrial.FixationOff>500  & GC_I==1 & InfoTrial.PreferD==Targ_dmax & InfoTrial.DOpt(:,Targ_dmax)==v;
  Rate(u)=nanmean(InfoTrial.DRate(I,Targ_dmax));
  AllTarg.C_ProV(unit_all,u,:)=nan(1,1002);
if sum(I)
  plotstd(t,Spike(I,:),char(Value_Color(u)));hold on;
% sum(DPrefer_V_trial==v  & InfoTrial.FixationOff>500)
  axis(clim);     
  title('2AFC  Chosen');  
  xlabel('From Target onset'); 
...
subplot(2,2,1) 
 ...
 subplot(2,2,3) 
  clear r
  r=squeeze(AllTarg.FC_ProV_RP(:,u,:));
  plotstd(t,r,char(Value_Color(u)));hold on;
 title('All FC  Chosen');  
 xlabel('From Target onset');
 axis(clim);  
