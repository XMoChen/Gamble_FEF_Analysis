...
load(Spikefile,'SaccadeRate','SaccadeHist','l_ts')
...
if l_ts(ch,unit)>1000
  ...
  a=nanmean(Spike(InfoTrial.ChoiceOpt==4 & GS_I==1 & InfoTrial.ChoiceD==4 & InfoTrial.TargetOn>500,:),1);
  clim=[-500 200 nanmin(a)*0.8 nanmax(a)*1.5]; 
  
%   clim=[ -500 200 mean(nanmean(Spike,1))-5*std(nanmean(Spike,1))   mean(nanmean(Spike,1))+10*std(nanmean(Spike,1))];
...
for v=V_all'
    u=u+1;
    InfoTrial.ChoiceOpt==v & GS_I==1 & InfoTrial.ChoiceD==d &
    I= InfoTrial.SaccadeStart>500 &  InfoTrial.ChoiceD==3;
...
for v=V_all'
    u=u+1;
    plotstd(t,Spike(InfoTrial.ChoiceOpt==v  & GC_I==1 & InfoTrial.ChoiceD==d & InfoTrial.TargetOn>500,:),char(Value_Color(u)));hold on;
% sum(DPrefer_V_trial==v  & InfoTrial.TargetOn>500)
    %axis([-600 50 0 inf])
  axis(clim);     
end
