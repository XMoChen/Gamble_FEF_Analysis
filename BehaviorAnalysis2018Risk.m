function  BehaviorAnalysis2018Risk(Eventfile,DirFig)

% valid InfoTrial.Reward>0
close all;
...
LowRisk_Result=[0;LowRisk_Result];
LowRisk_Result=LowRisk_Result(1:end-1);
HighRisk_Result=InfoTrial.Result(ID_Choice);
HighRisk_Result(LowRisk)=0;
HighRisk_Result=[0;HighRisk_Result];
HighRisk_Result=HighRisk_Result(1:end-1);
...
[I_Lw4L,I_Lw4R,I_LwL]=deal(zeros(1,size(InfoTrial_valid,1)));
I_Lw3R=I_LwL;
I_Lw3L=I_LwL;
I_LwR=I_LwL;
I_LwL=I_LwR;
...
BehaviorAlys.ChoicePerc=ChoicePerc(:);
BehaviorAlys.RiskPerc=RiskChoicePerc(3,:);
save(Eventfile,'BehaviorAlys','InfoTrial_valid','ChoicePerc','theta','-append');


clear X Z 
X(1,:)=InfoTrial_valid.Targ1Opt;
X(2,:)=InfoTrial_valid.Targ2Opt;
X_in(X(1,:)==2,1)=2;X_in(X(1,:)==2,2)=2;
X_in(X(1,:)==11,1)=0;X_in(X(1,:)==11,2)=4;
X_in(X(1,:)==12,1)=1;X_in(X(1,:)==12,2)=3;
X_in(X(2,:)==2,3)=2;X_in(X(2,:)==2,4)=2;
X_in(X(2,:)==11,3)=0;X_in(X(2,:)==11,4)=4;
X_in(X(2,:)==12,3)=1;X_in(X(2,:)==12,4)=3;
Z=InfoTrial_valid.ChoiceD;
I_choice=InfoTrial_valid.Targ2Opt>0;
Z_in(Z==1)=1;Z_in(Z==2)=0;
X0=[X_in(I_choice,:),Z_in(I_choice)'];
[logi_para,var_inact_aic,var_inact_bic,var_inact_rss]=V_risk(X0(:,1:4),X0(:,5)) ;
SubjV(1)=power(2,logi_para(1));
SubjV(2)=(power(1,logi_para(1))+power(3,logi_para(1)))/2;
SubjV(3)=(power(0,logi_para(1))+power(4,logi_para(1)))/2;
save(Eventfile,'logi_para','SubjV','RewardHis','-append');
