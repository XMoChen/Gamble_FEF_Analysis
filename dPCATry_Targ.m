    ...
    for ch=1:Ch_num0
        for unit=1:(max(find(l_ts(ch,:)>0)))
            if l_ts(ch,unit)>5000
                ...
                if 1==1%ttest(bg_a,bg_b)==1%max(nanmean(f,1))>2  & ttest(bg_a,bg_b)==1 & nanmean(bg_b)>1.1*nanmean(bg_a) & ttest(nanmean(bg_a1),nanmean(bg_a2))~=1%&  ttest(nanmean(bg(1:100,1:200),2), nanmean(bg(end-99:end,1:200),2))~=1% ttest(f0(I_choice,55),f0(I_choice,1))==1
                    unit_all=unit_all+1;
