    ...
    for ch=1:Ch_num0
        for unit=2:(max(find(l_ts(ch,:)>0)))
            if l_ts(ch,unit)>5000
                ...
                if 1==1%max(nanmean(f,1))>2  & ttest(bg_a,bg_b)==1 & nanmean(bg_b)>1.1*nanmean(bg_a) & ttest(nanmean(bg_a1),nanmean(bg_a2))~=1%&  ttest(nanmean(bg(1:100,1:200),2), nanmean(bg(end-99:end,1:200),2))~=1% ttest(f0(I_choice,55),f0(I_choice,1))==1
                    unit_all=unit_all+1;
%                     f0=(f0-nanmean(nanmean(bg(:,1:200),2),1))/nanstd(nanmean(bg(:,1:200),2));
                    ...
                    for choice=1:2
                        ...
                        for Opt1=1:3
                            ...
                            %I_choice
                            %%%% background
                            ID= I_choice & InfoTrial.ContrastBG==Opt1  & InfoTrial.ChoiceD==choice ; %& InfoTrial.ChoiceOpt==Opt1 ;
                            %                             sum(ID)
                            %%%% no background
                            ID_NC= I_NC  & InfoTrial.ContrastBG==Opt1  & InfoTrial.ChoiceD==choice ; %& InfoTrial.ChoiceOpt==Opt1 ;
                            %end
...
% t=downsample(-800:501,5);
% % t=downsample(-800:501,20);
% 
% color={'k','b','r'};
...
ContrastIndext=mean(b_all,1)>0;% & sum(b_all_sig<0.05,1)>0;
%%
% t=downsample(-500:501,20);
% color={'k','b','r'};
%
...
save('OzzyResultSpikeTargConSum.mat','ContrastIndext','firingRates_all','firingRates_NB_all','firingRatesAverage','firingRatesAverage_NB','trialNum_all','trialNum_NB_all');
