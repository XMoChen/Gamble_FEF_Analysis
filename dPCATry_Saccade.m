...
for record=1:length(Date)
    ...
    load(Spikefile,'l_ts','SaccadeEndRate','TargetRate','BackgroundRate','SelectSig','PreferD');
                    ...
                    for choice=1:2
                        ...
                        for Opt1=[2,12,11]
                           ...
                           if PreferD(ch,unit,1)==1
                            ID= I_choice & InfoTrial.ContrastBG>1 &  InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==choice & InfoTrial.ResultOn2>0; %& InfoTrial.ChoiceOpt==Opt1 ;
                            ID_NC= I_NC & InfoTrial.ContrastBG>1 &   InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==choice & InfoTrial.ResultOn2>0; %& InfoTrial.ChoiceOpt==Opt1 ;
                           else
                            ID= I_choice & InfoTrial.ContrastBG>1 &  InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==3-choice & InfoTrial.ResultOn2>0; %& InfoTrial.ChoiceOpt==Opt1 ;
                            ID_NC= I_NC & InfoTrial.ContrastBG>1 &   InfoTrial.ChoiceOpt==Opt1 & InfoTrial.ChoiceD==3-choice & InfoTrial.ResultOn2>0; %& InfoTrial.ChoiceOpt==Opt1 ;
                           end                    
...
% figure(100)
% subplot(121)
% imagesc(squeeze(firingRates_NB_all(116,2,3,t>0 & t<500,:))')
% subplot(122)
% plot(t(t>0 & t<500),squeeze(nanmean(firingRates_NB_all(116,2,3,t>0 & t<500,:),5)),'r');hold on
% plot(t(t>0 & t<500),squeeze(nanmean(firingRates_NB_all(116,2,2,t>0 & t<500,:),5)),'b');hold on
% plot(t(t>0 & t<500),squeeze(nanmean(firingRates_NB_all(116,2,1,t>0 & t<500,:),5)),'k');hold on
%%
% size(firingRatesAverage)
% ttest(nanmean(firingRatesAverage(:,1,1,1:25),4),nanmean(firingRatesAverage(:,1,3,1:25),4));
% anova1(squeeze(nanmean(nanmean(firingRatesAverage(:,1,:,t>100 & t<400),4),2)))
% anova1(squeeze(nanmean(nanmean(firingRatesAverage(:,2,:,t>100 & t<400),4),2)))
% anova1(squeeze(nanmean(nanmean(firingRatesAverage_NB(:,1,:,t>100 & t<400),4),2)))
% anova1(squeeze(nanmean(nanmean(firingRatesAverage_NB(:,2,:,t>100 & t<400),4),2)))
%%
a=squeeze(nanmean(nanmean(firingRatesAverage(:,1,:,t>100 & t<500),4),2));
...
a=squeeze(nanmean(nanmean(firingRatesAverage(:,2,:,t>100 & t<500),4),2));
b=ones(size(a,1),1)*[1 2 3];b1=b(:);
[b0,~,~,~,STATS]=regress(a(:),[ones(size(b1)),b1]);
% plot(b1,a(:),'.');hold on;
% plot(b1,[ones(size(b1)),b1]*b0);
...
a=squeeze(nanmean(nanmean(firingRatesAverage_NB(:,1,:,t>100 & t<500),4),2));
b=ones(size(a,1),1)*[1 2 3];b1=b(:);
[b0,~,~,~,STATS]=regress(a(:),[ones(size(b1)),b1]);
% plot(b1,a(:),'.');hold on;
% plot(b1,[ones(size(b1)),b1]*b0);
...
% 
% figure()
% a=squeeze(nanmean(nanmean(firingRatesAverage_NB(:,2,:,t>100 & t<500),4),2));
% b=ones(size(a,1),1)*[1 2 3];b1=b(:);
% [b0,~,~,~,STATS]=regress(a(:),[ones(size(b1)),b1]);
% plot(b1,a(:),'.');hold on;
% plot(b1,[ones(size(b1)),b1]*b0);
...
for n=1:size(firingRates_all,1)%36,110%1:size(firingRates_all,1)
 for direction=1:2
        ...
        clear a00** a a_NC
        a=squeeze(nanmean(firingRates_all(n,direction,:,t>100 & t<800,:),4))';
        a_NC=squeeze(nanmean(firingRates_NB_all(n,direction,:,t>100 & t<800,:),4))';
