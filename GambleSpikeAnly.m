function [FireAll,FireAll_C]=GambleSpikeAnly (Period,Eventfile,Spikefile,record)
load(Eventfile);
load(Spikefile);
...
for nu=1:Neu_num
  ...
  % nanmean(b1((end-199):end)-b1(1:200))< 0.1*nanmean(b1((end-199)))
    if  nanmean(b1((end-199):end)-b1(1:200))<0.2*nanmean(b1((end-199)))
        u=u+1;
...
%             end
            ...
            II= find( Reg.Repeat==0 & Reg.Contrast==contr & Reg.ChoiceD==d & Reg.ChoiceOpt==v & Reg.TrialType>=0 )
%             if length(II)<20
...
%             end
            ...
            if i_sw<10             
                II_1=[II(1:i_sw);II(randperm(i_sw));II(randperm(i_sw))];%II(i_sw)*ones(7,1)];
            else
            II_1=II(1:10);
            end
            II_10=II_1(1:10);
