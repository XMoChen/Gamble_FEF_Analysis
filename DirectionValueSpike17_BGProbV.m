function DirectionValueSpike17_BGProbV(Spikefile,Eventfile,DirFig,Ch_num)
%%%% dock the figures
close all
    ...
    for unit=2:length(l_ts(ch,:)) %3
        if l_ts(ch,unit)>5000
            ...
            if 1==1%hSig(unit_all,1)==0 & hSig(unit_all,3)==0 & hSig(unit_all,4)==0 & hSig(unit_all,5)==0% ...
                ...
                print( FigHandle, '-djpeg', [DirFig,'Ch',num2str(ch),'Unit',num2str(unit),'_BG2']);
...
print( FigHandle, '-djpeg', [DirFig,'_BG2']);
