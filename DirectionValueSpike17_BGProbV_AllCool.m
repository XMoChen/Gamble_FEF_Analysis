function DirectionValueSpike17_BGProbV_AllCool(Spikefile,Eventfile,DirFig,Ch_num)
%%%% dock the figures
close all
...
C1=(InfoTrial.Background<=6 & InfoTrial.Temperature<20);
ContrastBG(C1)=1;
C2=(InfoTrial.Background>6 & InfoTrial.Background<=12 & InfoTrial.Temperature<20 );
ContrastBG(C2)=2;
C3=(InfoTrial.Background>12 & InfoTrial.Temperature<20 );
ContrastBG(C3)=3;
...
%         if l_ts(ch,unit)>5000
            ...
            eval(['Spike=TargetRate.Ch',num2str(ch),'All;']);
            Spike_TG=Spike;
            eval(['Spike=SaccadeEndRate.Ch',num2str(ch),'All;']);
            Spike_SC=Spike;
            eval(['Spike=BackgroundRate.Ch',num2str(ch),'All;']);
            Spike_BG=Spike;
            eval(['Spike=ResultRate.Ch',num2str(ch),'All;']);
            Spike_RT=Spike;            
...
%             Spike_RT=Spike;
            ...
            if 1==1%hSig(unit_all,1)==0 & hSig(unit_all,3)==0 & hSig(unit_all,4)==0 & hSig(unit_all,5)==0% ...
                ...
                print( FigHandle, '-djpeg', [DirFig,'Ch',num2str(ch),'Unit',num2str(unit),'_Cool_BG2']);
...
print( FigHandle, '-djpeg', [DirFig,'_Cool_BG2']);
