...
for ch=1:Ch_num
    for unit=2:length(l_ts(ch,:))
        if l_ts(ch,unit)>10000
            ...
            Spike=Spike(I_valid,:);
            SpikeTrialMean.Targ(unit_all,:)=nanmean(Spike(:,550:1000),2);
          %  convn(InfoTrial_valid.LastRReward',kernel,'same')';
     %       a=convn(nanmean(Spike(:,550:1050),2)',kernel,'same')';
             a=Scale(nanmean(Spike(:,550:1000),2));
             
             X1=[InfoTrial_valid.DRate(:,1),LeftReward_Index,LeftRisk_Index,InfoTrial_valid.ContrastBG];        
             b = regress(a(FChoice_Index~=0 & InfoTrial_valid.DChoice(:,1)==1),X1(FChoice_Index~=0 & InfoTrial_valid.DChoice(:,1)==1,:));
...
subplot(442)
plot(InfoTrial_valid.DRate(FChoice_Index==0 & InfoTrial_valid.DChoice(:,1)==1,1),a(FChoice_Index==0 & InfoTrial_valid.DChoice(:,1)==1),'.');hold on; %plot(FChoice_Index*1.2,'k');
axis([0 inf min(a) max(a) ]);
...
subplot(446)
plot(LeftReward_Index(FChoice_Index==0  & InfoTrial_valid.DChoice(:,1)==1),a(FChoice_Index==0 & InfoTrial_valid.DChoice(:,1)==1),'.');hold on;% plot(FChoice_Index*4.5,'k');
axis([0 inf min(a) max(a)]); title('Left Saccade Reward');
box off;set(gca,'TickDir','out'); axis square
...
subplot(443)
plot(InfoTrial_valid.DRate(FChoice_Index==0 & InfoTrial_valid.DChoice(:,2)==1,1),a(FChoice_Index==0 & InfoTrial_valid.DChoice(:,2)==1),'.');hold on; %plot(FChoice_Index*1.2,'k');
axis([0 inf min(a) max(a) ]);
box off;set(gca,'TickDir','out'); title('Left Saccade Prob');
axis square
...
subplot(4,4,7)
plot(LeftReward_Index(FChoice_Index==0  & InfoTrial_valid.DChoice(:,2)==1),a(FChoice_Index==0 & InfoTrial_valid.DChoice(:,2)==1),'.');hold on;% plot(FChoice_Index*4.5,'k');
axis([0 inf min(a) max(a)]); title('Left Saccade Reward');
box off;set(gca,'TickDir','out'); axis square

subplot(4,4,11)
%plot(LeftRisk_Index(FChoice_Index~=0 & InfoTrial_valid.DChoice(:,1)==1),'k','LineWidth',1);hold on; plot(RightRisk_Index(FChoice_Index~=0),'b:','LineWidth',0.05);
plot(LeftRisk_Index(FChoice_Index==0  & InfoTrial_valid.DChoice(:,2)==1),a(FChoice_Index==0 & InfoTrial_valid.DChoice(:,2)==1),'.');hold on;% plot(FChoice_Index*4.5,'k');
axis([-0.5 2.5 min(a) max(a)]); title('Left Risk');
box off;set(gca,'TickDir','out'); axis square
