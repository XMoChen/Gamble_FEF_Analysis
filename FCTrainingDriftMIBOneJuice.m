...
function FCTrainingDriftMIBOneJuice
...
try
    ...
    pixperdeg=2*ppd;%Number of pixels that equal one degree of visual angle
    
    
  %Marc fix  
   %0 185 255
    fp_color=[0 0 0];
    tg_color=[150 150 150];
    fpr=round(ppd*0.5); %radius of fixation point
    tgr=round(ppd*2); %radius of fixation point

 %   %      green=[0 200 0];
 %      blue=[0 185 255];
    
    
    window_fix_ini=round(10*ppd);
    window_fix=round(1.75*ppd); % fixation window (diameter)
    window_choice=round(7*ppd);
    single_pos=0;
    pos=1;
    ...
    t_overlap0=0.200;   %  required fixation time
    t_overlaprand=0.1;
    t_trialend2=0.50;   
    t_image_pop2=0.5;
    t_blank2=0.5;
...
%         5*ones(1,OnsetLength),6*ones(1,OnsetLength),7*ones(1,OnsetLength),8*ones(1,OnsetLength)];
  ...
  OverType=unique(I_overlap0);
      ...
      Orientation_squence=[ 1*ones(1,OnsetLength),2*ones(1,OnsetLength),3*ones(1,OnsetLength),4*ones(1,OnsetLength),...
    1*ones(1,OnsetLength),2*ones(1,OnsetLength),3*ones(1,OnsetLength),4*ones(1,OnsetLength)];
    I0=randperm(length(I_squence));
...
% Connection with Eyelink if not in testing mode
 ...
 phaseStep = 10; % affects the speed of movement
...
%     t1MovOriginal(i) = Screen('MakeTexture',w,frame);
      m=Grating(phase,size_rect_gabor,size_gabor,pixperdeg);
      c=0.1;
      inc=255-gray;
      MIBFrame(i)=Screen('MakeTexture', w, gray+inc*m*c);
...
%    display(['RtR: ',num2str(median(RtSumR))]);
 ...
 Eyelink('Message','Direction %d', Direct);
  MIB_Direct=Orientation_squence0(i_onset);

 Eyelink('Message','OnsetDif %ld', round(offtime*1000));
 Eyelink( 'Message', 'Overlaptime %ld', round( t_overlap*1000 ));
...
% 	t_start_trial=Screen('Flip', w, t_start_trial + t1 - slack);
 ...
 [x_eye,y_eye,pause,corr_x,corr_y]=checkkeys3(keyIsDown,keyCode,pause,x_eye,y_eye,center,x_fp,y_fp,corr_x,corr_y);
    ...
    if sqrt(corr_x^2+corr_y^2)<=((window_fix_ini/4))
    gaze_x=x_fp+corr_x;
    gaze_y=y_fp-corr_y;
 ...
 if  Direct<=4
    ...
    Screen('FillRect',w, gray);
    Screen('FillOval',w,fp_color, [center(1)-fpr+x_fp, center(2)-fpr-y_fp, center(1)+fpr+x_fp, center(2)+fpr-y_fp]); 
    Screen('FillOval',w,tg_color, [center(1)-tgr+x_fp1, center(2)-tgr-y_fp1, center(1)+tgr+x_fp1, center(2)+tgr-y_fp1]);   
    Screen('FillOval',w,tg_color, [center(1)-tgr+x_fp2, center(2)-tgr-y_fp2, center(1)+tgr+x_fp2, center(2)+tgr-y_fp2]); 
...
%            Repeat_flag=0;
    ...
    while ( fix1==1 & fix==1 && fix2==0 && choice==0 && ((GetSecs)-t_start)< t_waitforchoice)
   ...
   %Check recording status, stop display if error
    ...
    if(error~=0)
     ...
     if ((x_eye >= (center(1)+gaze_x+x_fp1-(window_choice/2)))&&(x_eye <= (center(1)+gaze_x+x_fp1+(window_choice/2)))&&(y_eye >= (center(2)-gaze_y-y_fp1-(window_choice/2)))&&(y_eye <= (center(2)-gaze_y-y_fp1+(window_choice/2))))
     choice=1;  
    ...
    RtSumR=[RtSumR;(ChoiceIn-FixOn)];
     ...
     if Direct<5 &  ((x_eye >= (center(1)+gaze_x+x_fp2-(window_choice/2)))&&(x_eye <= (center(1)+gaze_x+x_fp2+(window_choice/2)))&&(y_eye >= (center(2)-gaze_y-y_fp2-(window_choice/2)))&&(y_eye <= (center(2)-gaze_y-y_fp2+(window_choice/2))))
     choice=2;
     Eyelink('Message', 'ChoiceIn');
...
%      disp('Choice');
 ...
 if isempty(OffSum)==1
    ...
    while (choice>0 && choice<100 && choice2 ==choice &&((GetSecs)-t_start)<t_choice),
     ...
     if   ((x_eye >= (center(1)+gaze_x+x_fp2-(window_choice/2)))&&(x_eye <= (center(1)+gaze_x+x_fp2+(window_choice/2)))&&(y_eye >= (center(2)-gaze_y-y_fp2-(window_choice/2)))&&(y_eye <= (center(2)-gaze_y-y_fp2+(window_choice/2))))
     choice2=2;
     end
    ...
    if ( choice~=0 & fix==1 &  fix2==0  & fix1==1  & choice2>0 )
             reward=1;
             data.trialaccepted(i)=1;
             Eyelink( 'Message', 'Reward %d', reward);
             giveJuice_GN(2);  
