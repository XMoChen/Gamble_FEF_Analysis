...
try
 ...
 %      blue=[0 185 255];
    ...
    t_fixation2=0.200;   %  required fixation time
    t_overlaprand=0.1;
    t_trialend2=0.750;   
    t_image_pop2=0.5;
    t_blank2=0.5;
...
%         5*ones(1,OnsetLength),6*ones(1,OnsetLength),7*ones(1,OnsetLength),8*ones(1,OnsetLength)];
  ...
  I_overlap0=mod(I_overlap,4)*0.1;
...
%     Eyelink( 'Message', 'Central Y %ld',Attention_Loc);
 ...
 while (i+1)<=round(length(Type_ID))
    ...
    display(['Fixation=',num2str(t_overlap+t_fixation2)]);
