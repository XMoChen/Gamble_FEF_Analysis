function [FireAll,FireAll_C]=EventAnlyLFP (Eventfile,LFPfile,record,Ch_num,event,t_interval)
load(Eventfile);
load(LFPfile);
...
elseif strcmp(event,'Saccade')==1
    eval(['a= LFP_Saccade_',num2str(nu),';'])   
elseif strcmp(event,'Result')==1
    eval(['a= LFP_Result_',num2str(nu),';'])   
elseif strcmp(event,'Background')==1
    eval(['a= LFP_Background_',num2str(nu),';'])   
elseif strcmp(event,'SaccadeEnd')==1
    eval(['a= LFP_SaccadeEnd_',num2str(nu),';'])   
    end
...
%%
lim=[-t_interval(1) t_interval(2) min(FireMean(:))*0.9 max(FireMean(:))*1.2];
t=-t_interval(1):size(LFP_Result,3)-t_interval(1)-1;
figure()
