function FireAll=BGAnly (Eventfile,Spikefile,record)
load(Eventfile);
load(Spikefile);
...
for nu=1:Neu_num
    u=u+1;
    clear a b
 %   eval(['a= SaccadeEndRate0{',num2str(TF(nu)),'};'])   
...
%      b=b(I_select,:);
   ...
   [h,p]=ttest2(b1(1:100), b0((end-99):end))
    if nanmean(a(:))>5 & p>0.05
        SaccadeEndRate1(u,:,:)= (a-(b1(:)))/std(b1(:));
    end
end
...
for contr=1:3
    for d=1:2
        ...
        for v=[2,12,11]
            ...
            FireAll(contr,d,u,:,1:10,:)=squeeze(SaccadeEndRate1(:,II,:));
...
%%
lim=[-100 500 min(FireMean(:))*0.9 max(FireMean(:))*1.2];
t=-100:size(SaccadeEndRate1,3)-101;
figure(record)
