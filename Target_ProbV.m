function Target_ProbV(Spikefile,Eventfile,DirFig,Ch_num,name0,Bg_Lable)
%%%% dock the figures
    ...
    for unit=2:length(l_ts(ch,:)) %3
        if l_ts(ch,unit)>5000
            ...
            %
                      ...
                      if hSig(unit_all,9)==1 & hSig(unit_all,10)==1 & nanmean(nanmean(Spike_TG(I2,1000:1300),2),1)>nanmean(nanmean(Spike_TG(I1,1000:1300),2),1)
                      PreferD(ch,unit)=2;
                      else
                      PreferD(ch,unit)=1;
                      end
            ...
            if 1==1%hSig(unit_all,1)==0 & hSig(unit_all,3)==0 & hSig(unit_all,4)==0 & hSig(unit_all,5)==0% ...
                ...
                figure(unit_all)
                Epoch={'BG','TG','SC','RT'};
                Spike_max=max(nanmean(Spike_TG,2));
                Spike_min=min(nanmean(Spike_TG,2));
                ...
                clear Spike tt
                eval(['Spike=(Spike_TG-Spike_min)/(Spike_max-Spike_min);']);
                %                 Spike_max=1;
                %                 Spike_min=0.5;
                I= InfoTrial.Result>0 & InfoTrial.Repeat==0 & GS_I==1 ;
                
                Spike_max=max(nanmean(Spike(I,:),1));
                Spike_min=min(nanmean(Spike(I,:),1));
                t=-1000:501;
                clim=[-800 500 Spike_min*0.8 Spike_max*1.5];
                ...
                for d=1:2%4
                    ...
                    for v=[4,21,22,23,24,25]
                        ...
                        if (v==4 | v==21 | v==25)
                            subplot(2,4,d)
                            clear I
                            I= InfoTrial.Result>0 & InfoTrial.Repeat==0 & GS_I==1 & InfoTrial.ChoiceD==d & InfoTrial.ChoiceOpt==v;
                            SpikeTarget(unit_all,d,u,:)=nanmean(Spike(I,:),1);
                        ...
                        if u==1 & d==1
                            xlabel(['From target onset']); %title('Choice');
                            if Bg_Lable==1
                ...
                for d=1:2%4
                    ...
                    for v=[4,21,22,23,24,25]
                        ...
                        if (v==4 | v==21 | v==25)
                            subplot(2,4,d+4)
                            clear I
                            I= InfoTrial.Result>0 & InfoTrial.Repeat==0 & GC_I==1 & InfoTrial.ChoiceD==d & InfoTrial.ChoiceOpt==v;
                            SpikeTarget(unit_all,d+2,u,:)=nanmean(Spike(I,:),1);
                            plotstd(t,Spike(I,:),char(V_Color(u)));hold on;
                            box off;set(gca,'TickDir','out')
                            axis(clim);
                ...
                xlabel(['From Saccade Offset']); %title('Choice');
                
                
                FigHandle = figure(unit_all);
                print( FigHandle, '-djpeg', [DirFig,'Ch',num2str(ch),'Unit',num2str(unit),'_Targ']);
                %
                %
...
%
h2=figure;
aclim=nanmean(SpikeTarget,1);
clim=[-800 500 min(aclim(:)), max(aclim(:))];
for d=1:2
    subplot(2,4,d)
    
    plotstd(t,squeeze(SpikeTarget(:,d,2,:)),'g');
    plotstd(t,squeeze(SpikeTarget(:,d,1,:)),'m');
    plotstd(t,squeeze(SpikeTarget(:,d,6,:)),'r');
    axis(clim);
    box off;set(gca,'TickDir','out')
    ...
    subplot(2,4,d+2)
    plotstd(t,squeeze(SpikeTarget(:,d,3,:)),'k');
    plotstd(t,squeeze(SpikeTarget(:,d,4,:)),'c');
    plotstd(t,squeeze(SpikeTarget(:,d,5,:)),'b');
    axis(clim);
    box off;set(gca,'TickDir','out')
    title(['FC D',num2str(d)])
    ...
    subplot(2,4,d+4)
    plotstd(t,squeeze(SpikeTarget(:,d+2,2,:)),'g');
    plotstd(t,squeeze(SpikeTarget(:,d+2,1,:)),'m');
    plotstd(t,squeeze(SpikeTarget(:,d+2,6,:)),'r');
    axis(clim);
    box off;set(gca,'TickDir','out')
    title(['Choice D',num2str(d)])
    
    subplot(2,4,d+6)
    plotstd(t,squeeze(SpikeTarget(:,d+2,3,:)),'k');
    plotstd(t,squeeze(SpikeTarget(:,d+2,4,:)),'c');
    plotstd(t,squeeze(SpikeTarget(:,d+2,5,:)),'b');
    axis(clim);
    box off;set(gca,'TickDir','out')
    title(['Choice D',num2str(d)])
...
if Bg_Lable==1
    ...
    BG_SpikeTarget=SpikeTarget;
    save(Spikefile,'BG_SpikeTarget','-append');
    print( h2, '-djpeg', [DirFig,'Targ_',name0,'_BG']);
    print( h2, '-djpeg', [sumfig,'Targ_',name0,'_BG']);
    
else
    suptitle(['NBG ',char(name0)]);
    NBG_SpikeTarget=SpikeTarget;
    save(Spikefile,'NBG_SpikeTarget','-append');
    print( h2, '-djpeg', [DirFig,'Targ_',name0,'_NBG']);
    print( h2, '-djpeg', [sumfig,'Targ_',name0,'_NBG']);
    
end
% print( h1, '-djpeg', [DirFig,'1_',name0]);
% print( h2, '-djpeg', [DirFig,'2_',name0]);
% print( h1, '-djpeg', [sumfig,'Targ1_',name0]);
% %%
% %===============================================
