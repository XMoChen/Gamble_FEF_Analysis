...
if l_ts(ch,unit)>5000
  ...
  I1= InfoTrial.Repeat==0 & InfoTrial.TargetOn>500 & GC_I==1 &  ContrastBG==2 & InfoTrial.DOpt(:,Targ_dmax)==2;
  I2= InfoTrial.Repeat==0 & InfoTrial.TargetOn>500 & GC_I==1 &  ContrastBG==3 & InfoTrial.DOpt(:,Targ_dmax)==18;
...
subplot(2,3,1) 
 ...
 I= InfoTrial.Repeat==0 & InfoTrial.TargetOn>500 & GS_I==1 &  ContrastBG>=2 ;
 AllBG.FC_ProV_BG(unit_all,u,:)=nan(1,602);
 if sum(I)>5
...
subplot(2,3,4) 
 I= InfoTrial.Repeat==0 & InfoTrial.TargetOn>500 & GS_I==1 &  ContrastBG==1 ;
AllBG.FC_ProV_BK(unit_all,u,:)=nan(1,602);
 if sum(I)>5
 plotstd(t,Spike(I,:),char(Value_Color(u)));hold on;
 title('FC  Blank');  
 xlabel('From Background onset');
 ...
 AllBG.FC_ProV_BK(unit_all,u,:)=(nanmean(Spike(I,:),1)-Spike_min)/(Spike_max-Spike_min);
  ...
  subplot(2,3,2) 
  clear I
  I= InfoTrial.Repeat>-1 & InfoTrial.TargetOn>500  & ContrastBG>=2 & GC_I==1 & InfoTrial.PreferD==Targ_dmax & InfoTrial.DOpt(:,Targ_dmax)==v;
  Rate(u)=nanmean(InfoTrial.DRate(I,Targ_dmax));
AllBG.C_ProV_BG(unit_all,u,:)=nan(1,602);
 if sum(I)>5
  plotstd(t,Spike(I,:),char(Value_Color(u)));hold on;
% sum(DPrefer_V_trial==v  & InfoTrial.TargetOn>500)
  ...
  clear I
  I= InfoTrial.Repeat>-1 & InfoTrial.TargetOn>500 & ContrastBG>=2 & GC_I==1 & InfoTrial.PreferD~=Targ_dmax & InfoTrial.DOpt(:,Targ_dmax)==v;
  AllBG.C_AwyV_BG(unit_all,u,:)=nan(1,602);
  if sum(I)>5
  plotstd(t,Spike(I,:),char(Value_Color(u)));hold on;
% sum(DPrefer_V_trial==v  & InfoTrial.TargetOn>500)
  axis(clim);     
  ...
  AllBG.C_AwyV_BG(unit_all,u,:)=(nanmean(Spike(I,:),1)-Spike_min)/(Spike_max-Spike_min);
    ...
    subplot(2,3,3) 
  clear I
  AllBG.C_ProV_BK(unit_all,u,:)=nan(1,602);
...
% sum(DPrefer_V_trial==v  & InfoTrial.TargetOn>500)
  ...
  subplot(2,3,6) 
  clear I
  AllBG.C_AwyV_BK(unit_all,u,:)=nan(1,602);
  I= InfoTrial.Repeat>-1 & InfoTrial.TargetOn>500 & ContrastBG==1 & GC_I==1 & InfoTrial.PreferD~=Targ_dmax & InfoTrial.DOpt(:,Targ_dmax)==v;
  if sum(I)>5
  plotstd(t,Spike(I,:),char(Value_Color(u)));hold on;
% sum(DPrefer_V_trial==v  & InfoTrial.TargetOn>500)
