...
Result_FireFFTSum=[];
start_t0=1050;
start_control=500; % before background onest as baseline
for file_i=1:length(fileList)
    filename=fileList(file_i).name;
    clear ResultFireAll activity_m Targ_FireAll Result_firing_sub
    load([main_dir,filename],'Targ_FireAll');
    Result_FireAll=Targ_FireAll;
    figure(file_i)
...
%     end
    ...
    Result_firing_control=Result_FireAll(:,:,:,:,:,(start_control+1):(start_t0+256));
            ...
            for d=1:size(Result_firing_sub,3)
                for v=1:size(Result_firing_sub,4)
                    for trial=1:size(Result_firing_sub,5)
                        ...
                        lfp_control=squeeze(Result_firing_control(chan,contra,d,v,trial,:));
                        ...
                        if isnan(lfp_tiral(1))==1 | nanmean(lfp_tiral)==0
                            ...
                            trial_fft=log((((pmtm(lfp_tiral,2.5,NFFT,Fs,'eigen')+10^-50))));%...
                             %   -log((((pmtm(lfp_control,2.5,NFFT,Fs,'eigen')+10^-50))));
