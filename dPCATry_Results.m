...
%                  f=reshape(zscore(f(:)),size(f,1),size(f,2));%ScaleMean(f);%
                f0=downsample(f',20)';
                if max(nanmean(f0,1))>2  & ttest(nanmean(bg(1:100,1:200),2), nanmean(bg(end-99:end,1:200),2))~=1%& ttest(f0(I_choice,55),f0(I_choice,1))==1
                    unit_all=unit_all+1;
                    ...
                    for choice=1:2
                        ...
                        for Opt1=[2,12,11]
                            ...
                            %                                 else
                            %%%% background
                            ID=  InfoTrial.ContrastBG==3 & InfoTrial.ChoiceD==choice & InfoTrial.Reward>0 & InfoTrial.ChoiceOpt==Opt1 ;
                            %%%% no background
                            ID_NC=  InfoTrial.ContrastBG==1 & InfoTrial.ChoiceD==choice & InfoTrial.Reward>0 & InfoTrial.ChoiceOpt==Opt1 ;
                            %                                 end
