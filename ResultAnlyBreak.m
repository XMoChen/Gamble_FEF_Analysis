function [FireAll,FireAll_C]=ResultAnlyBreak (Eventfile,Spikefile,record)
load(Eventfile);
load(Spikefile);
...
%             end
            ...
            FireAll(contr,d,u,nu,1:20,:)=squeeze(TargetRate1(nu,II(1:20),:));
            BehaveFireAll(contr,d,u,nu,1,1:20)=InfoTrial(II(1:20),16);
            BehaveFireAll(contr,d,u,nu,2,1:20)=InfoTrial(II(1:20),17);            
            FireAll_C(contr,d,u,nu,1:80,:)=nan(80,size(TargetRate1,3));
            BehaveFireAll_C(contr,d,u,nu,1:2,1:80)=nan(2,80);
            if length(II_C)>0  
            FireAll_C(contr,d,u,nu,1:length(II_C),:)=squeeze(TargetRate1(nu,II_C,:));
            BehaveFireAll_C(contr,d,u,nu,1,1:length(II_C))=InfoTrial(II_C,16);
            BehaveFireAll_C(contr,d,u,nu,2,1:length(II_C))=InfoTrial(II_C,17); 
            end
        end
    end
end
