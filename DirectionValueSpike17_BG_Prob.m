function DirectionValueSpike17_BG_Prob(Spikefile,Eventfile,DirFig,Ch_num)
%%%% dock the figures
close all
    ...
    for unit=2:length(l_ts(ch,:))
        if l_ts(ch,unit)>5000
            ...
            for ep=1:4
                ...
                if ep==1
                    t=-100:501;
                    clim=[-100 500 Spike_min*0.8 Spike_max*1.5];
                elseif ep==2
                    t=-700:501;
                    clim=[-700 500 Spike_min*0.8 Spike_max*1.5];
                elseif ep==3
                    t=-500:801;
                    clim=[-500 600 Spike_min*0.8 Spike_max*1.5];
                else
                    t=-500:501;
                    clim=[-300 400 Spike_min*0.8 Spike_max*1.5];
                end
...
for ep=1:4
    ...
    xlabel(['From ',char(Epoch(ep)),' onset']);% title('Single');
        ...
        for v=[2,16,17,18]
            u=u+1;
            clear I
            I= ContrastBG>1 &  InfoTrial.Repeat>-1 & InfoTrial.SaccadeStart>500 & GS_I==1 & InfoTrial.ChoiceD==d & InfoTrial.ChoiceOpt==v;
    ...
    for d=1:2%4
        ...
        for v=[2,16,17,18]
            u=u+1;
            clear I
            I= ContrastBG>1 & InfoTrial.Repeat>-1 & InfoTrial.SaccadeStart>500 & GC_I==1 & InfoTrial.ChoiceD==d & InfoTrial.ChoiceOpt==v;
            plotstd(t,squeeze(nanmean(Spike(:,I,:),2)),char(V_Color(u)));hold on;
        end
        axis(clim);
    end
